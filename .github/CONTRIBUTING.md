...
---
The section below is specific to the **Generic Browser Automation Framework** project. It explains how we work, how we use GitHub, and what contributors can expect when engaging with this repository.
---
...
Commit messages for this project follow the conventions below. Here is an example:

'''
feat(automation_system): add support for parallel execution

A new feature is added to the AutomationSystem to allow running browser instances in parallel, improving performance for large-scale tasks.
...
The name of the package affected by the change (e.g., `config`, `app_handlers`), and should be provided in parentheses before the colon.

#### description

A short one-line summary of the change, that should be written to complete the sentence "This change modifies the framework to ..." That means it does not start with a capital letter, is not a complete sentence, and actually summarizes the result of the change. Note that the verb after the colon is lowercase, and there is no trailing period
